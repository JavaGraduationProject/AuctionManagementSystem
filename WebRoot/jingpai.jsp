...
if (a.equals("insert")) {
    ...
    String charuid = new CommDAO().insert(request, "jingpai", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    new CommDAO()
        .commOper(
            "UPDATE shangpin SET dangqianjingjia='" +
            request.getParameter("jingpaijia") +
            "',jingpairen='" +
            request.getParameter("jingpairen") +
            "' WHERE id='" +
            request.getParameter("shangpinid") +
            "'"
        );
    ...
    String charuid = new CommDAO().update(request, "jingpai", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    Map map = Query.make("jingpai").find(Request.get("id"));

    new CommDAO().delete(request, "jingpai"); // 执行删除命令
