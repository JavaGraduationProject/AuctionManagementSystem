...
if (a.equals("insert")) {
    ...
    String charuid = new CommDAO().insert(request, "fenlei", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    ...
    String charuid = new CommDAO().update(request, "fenlei", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    Map map = Query.make("fenlei").find(Request.get("id"));

    new CommDAO().delete(request, "fenlei"); // 执行删除命令
